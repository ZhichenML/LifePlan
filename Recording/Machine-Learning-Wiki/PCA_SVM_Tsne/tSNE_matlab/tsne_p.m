...
% University of California, San Diego
    ...
    for iter=1:max_iter
        ...
        if ~rem(iter, 10) && ~isempty(labels)
            ...
            elseif no_dims == 2
%                 for la = 1 : 6
%                     scatter(ydata(find(labels==la),1),ydata(find(labels==la),2),'filled');hold on;
%                 end
                set(gca,'linewidth', 0.5, 'box', 'off', 'fontsize', 14,'FontName','Times New Roman',...
    'Position',[0.06,0.07,0.92,0.90]);
                hold off;
                scatter(ydata(:,1), ydata(:,2), 19, labels,'filled');%, 
            else
                for la = 1 : 6
                    scatter3(ydata(find(labels==la),1),ydata(find(labels==la),2),ydata(find(labels==la),3),'filled');hold on;
                end
                hold off;
